...
var MAC={
    ...
    'Login':{
        ...
        'Create':function($id){
            ...
            html = '<div class="{$maccms.cache_flag} drop-box login_box" id="login_box" style="display: none;">';
            if(MAC.Cookie.Get('userid')!=undefined && MAC.Cookie.Get('userid')!=''){ 
                html+='<ul class="{$maccms.cache_flag} logged"><li><a target="_blank" href="'+SitePath+'index.php?m=user-index">我的资料</a></li><li class="{$maccms.cache_flag} logout"><a class="{$maccms.cache_flag} logoutbt" href="javascript:;" onclick="MAC.Login.Out(\''+$id+'\');" target="_self"><i class="{$maccms.cache_flag} ui-icon user-logout"></i>退出</a></li></ul>';
            }
            else{
                html+='<form id="loginform" onsubmit="return false;" action="'+SitePath+'index.php?user-check" method="post"><div class="{$maccms.cache_flag} formitem"><label>用户：</label><input name="username" type="text"  class="{$maccms.cache_flag} input" id="username"/></div><div class="{$maccms.cache_flag} formitem"><label>密码：</label><input name="userpass" type="password" class="{$maccms.cache_flag} input" id="userpass"/></div><div class="{$maccms.cache_flag} formitem"><a class="{$maccms.cache_flag} qq-login" href="'+SitePath+'index.php?m=user-reg-ref-qqlogin.html"></a> <input class="{$maccms.cache_flag} formbutton" id="loginbtn" type="submit" value="登 录"></div><div class="{$maccms.cache_flag} formitem"><a title="忘记密码" class="{$maccms.cache_flag} forgotpass" href="'+SitePath+'index.php?m=user-findpass.html">忘记密码?</a>  <a class="{$maccms.cache_flag} reg-btn" href="'+SitePath+'index.php?m=user-reg.html" target="_blank">还没有账号?</a></div></form>';
            }
    ...
    'History': {
        ...
        'Create': function($id){
            ...
            html = '<div id="history_box" class="{$maccms.cache_flag} dropdown right history clearfix">';
            html +='<div class="{$maccms.cache_flag} head"><div class="{$maccms.cache_flag} pull-right"><a target="_self" href="javascript:void(0)" onclick="MAC.History.Clear();">清空</a> <a target="_self" href="javascript:void(0)" onclick="MAC.History.Hide();">关闭</a></div>观看历史</div><ul class="{$maccms.cache_flag} list" style="width:200px">';
            if(jsondata.length > 0){
                for($i=0; $i<jsondata.length; $i++){
                    if($i%2==1){
                        html +='<li class="{$maccms.cache_flag} text-overflow">';
                    }else{
                        html +='<li class="{$maccms.cache_flag} text-overflow">';
                    }
                    html +='<a href="'+jsondata[$i].link+'"><span class="{$maccms.cache_flag} text-muted">'+$i+'.</span> '+jsondata[$i].name+'</a></li>';
                }
            }else{
    ...
    'Score': {
        ...
        'Show':function($f,$tab,$id){
            ...
            if($f==1){
                ...
                str += '<div class="{$maccms.cache_flag} star score clearfix"><ul><li class="{$maccms.cache_flag} star_current"></li>';
                for(var i=1;i<=10;i++){ str += '<li><a value="'+i+'" class="{$maccms.cache_flag} star_'+i+'">'+i+'</a></li>'; }
                str += '<span id="star_tip"></span><span id="star_hover"></span></ul>';
                str +='<p class="{$maccms.cache_flag} branch"><span id="star_shi">0</span><span id="star_ge">.0</span><span class="{$maccms.cache_flag} star_no">(已有<label id="star_count">0</label>人评分)</span></p></div>';
            }
            document.write(str);
        ...
        'View':function($r){
            ...
            if ($("#star_shi").html()==0) {
                $('.branch').html('<span class="{$maccms.cache_flag} font-size-12">暂无评分</span>')
            }else{
                $("#star_shi").text( parseInt($r.score) );
            }
        }
