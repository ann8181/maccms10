...
function markscore0(vd,d,t,s,l,ac){
	...
	document.write('<div style="padding:5px 10px;border:1px solid #CCC">\
			...
			<div><span class="{$maccms.cache_flag} branch" id="MARK_B4"></span></div>\
			<div>');
...
function markscore1(vd,d,t,s,l,ac){
	...
	document.write('</span>');
	document.write('<span class="{$maccms.cache_flag} branch" id="MARK_B2"></span>');
	...
	dc.onmouseout=function (){
		...
		document.getElementById('MARK_B2').innerHTML=y>0 ? alt[parseInt(y/l*(alt.length-1))] : '<span class="{$maccms.cache_flag} font-size-12">请选择</span>' ;
	}
	if(new Date().toGMTString()!=new Date(document.lastModified).toGMTString()) return ajax.get(url[0],function (obj){
		var a=obj.responseText
		try{
